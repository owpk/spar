...
import ru.sparural.engine.loymax.rest.dto.Actions;
...
import ru.sparural.engine.loymax.rest.dto.LoymaxCounterValueResponse;
import ru.sparural.engine.loymax.rest.dto.LoymaxCouponsDto;
import ru.sparural.engine.loymax.rest.dto.LoymaxDataResultResponse;
...
import ru.sparural.engine.loymax.rest.dto.cards.LoymaxCardAttachStatusDto;
import ru.sparural.engine.loymax.rest.dto.cards.LoymaxCardReplaceResponse;
import ru.sparural.engine.loymax.rest.dto.cards.LoymaxCardSetStatus;
import ru.sparural.engine.loymax.rest.dto.cards.LoymaxUserCardDto;
import ru.sparural.engine.loymax.rest.dto.cards.LoymaxUserCardQrDto;
...
import ru.sparural.engine.loymax.rest.dto.registration.BeginRegistration;
import ru.sparural.engine.loymax.rest.dto.registration.CompleteRegistrationResponse;
import ru.sparural.engine.loymax.rest.dto.registration.DeregistrationConfirmCodeLengthDto;
import ru.sparural.engine.loymax.rest.dto.registration.DeregistrationConfirmDto;
import ru.sparural.engine.loymax.rest.dto.registration.RegistrationBeginResponse;
import ru.sparural.engine.loymax.rest.dto.status.LoymaxUserStatus;
import ru.sparural.engine.loymax.rest.dto.user.LoymaxFixedAnswer;
import ru.sparural.engine.loymax.rest.dto.user.LoymaxQuestionDataRoot;
...
public class LoymaxRestClient {
    ...
    public LoymaxAttributesDto<String> importUserAttribute(String adminToken, Long loymaxUserId, String attributeName) {
        var url = loymaxConstants.getGetAttributeValue().replace("{userId}", String.valueOf(loymaxUserId)) +
                "?logicalName=" + attributeName;
        var resp = restTemplate.request()
                .withFailureCallback(this::handleLoymaxErrors)
                .withAuthorizationHeader(AuthType.BEARER, adminToken)
                .setResponseType(new TypeReference<LoymaxDataResultResponse<LoymaxAttributesDto<String>>>() {
                })
                .getForEntity(url)
                .orElseThrow(this::throwUnexpectedEmptyResponseException);
        validateResponseState(resp);
    ...
    public List<Object> getUserInfo(String adminToken, String phoneNumber) {
        var url = loymaxConstants.getUserInfo() + "?phone=" + phoneNumber;
        var resp = restTemplate.request()
                .withFailureCallback(this::handleLoymaxErrors)
                .withAuthorizationHeader(AuthType.BEARER, adminToken)
                .setResponseType(new TypeReference<LoymaxDataResultResponse<List>>() {
                })
                .getForEntity(url)
                .orElseThrow(this::throwUnexpectedEmptyResponseException);
        validateResponseState(resp);
