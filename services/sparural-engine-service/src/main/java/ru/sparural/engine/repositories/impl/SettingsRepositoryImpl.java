...
public class SettingsRepositoryImpl implements SettingsRepository {
    ...
    public Setting update(Setting data) {
        var update = dslContext.update(Settings.SETTINGS)
                ...
                .set(Settings.SETTINGS.NOTIFICATIONSFREQUENCY, data.getNotificationsFrequency())
                .set(Settings.SETTINGS.UPDATEDAT, TimeHelper.currentTime())
                .returning().fetchOne();
        if (update == null)
            return null;
    ...
    private Setting createEmpty() {
        var create = dslContext.insertInto(Settings.SETTINGS)
                ...
                .set(Settings.SETTINGS.NOTIFICATIONSFREQUENCY, 0)
                .set(Settings.SETTINGS.CREATEDAT, TimeHelper.currentTime())
                .returning().fetchOne();
        if (create == null)
            return null;
