...
public class LoymaxSettingsRepositoryImpl implements LoymaxSettingsRepository {
    ...
    public LoymaxSetting update(LoymaxSetting data) {
        var update = dslContext.update(LoymaxSettings.LOYMAX_SETTINGS)
                ...
                .set(LoymaxSettings.LOYMAX_SETTINGS.MAXFAFORITECATEGORIESCOUNT, data.getMaxFaforiteCategoriesCount())
                .set(LoymaxSettings.LOYMAX_SETTINGS.SITEKEY, data.getSiteKey())
                .returning().fetchOne();
        if (update == null)
            return null;
    ...
    public void updateMaxCount(Long maxCount) {
        ...
        dslContext.update(LoymaxSettings.LOYMAX_SETTINGS)
                .set(LoymaxSettings.LOYMAX_SETTINGS.MAXFAFORITECATEGORIESCOUNT, maxCount)
                .returning().fetchOne();
    }
    ...
    private LoymaxSetting createEmpty() {
        var create = dslContext.insertInto(LoymaxSettings.LOYMAX_SETTINGS)
                ...
                .set(LoymaxSettings.LOYMAX_SETTINGS.MAXFAFORITECATEGORIESCOUNT, 0L)
                .returning().fetchOne();
        if (create == null)
            return null;
