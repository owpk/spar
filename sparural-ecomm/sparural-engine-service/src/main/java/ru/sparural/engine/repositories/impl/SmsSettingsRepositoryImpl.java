...
public class SmsSettingsRepositoryImpl implements SmsSettingsRepository {
    ...
    public SmsSetting update(SmsSetting data) {
        var update = dslContext.update(SmsSettings.SMS_SETTINGS)
                ...
                .set(SmsSettings.SMS_SETTINGS.SENDER_NAME, data.getSenderName())
                .set(SmsSettings.SMS_SETTINGS.GATEWAY_LOGIN, data.getGatewayLogin())
                .set(SmsSettings.SMS_SETTINGS.GATEWAY_PASSWORD, data.getGatewayPassword())
                .set(SmsSettings.SMS_SETTINGS.UPDATED_AT, new Date().getTime())
                .returning().fetchOne();
        if (update == null)
            return null;
    ...
    private SmsSetting createEmpty() {
        var create = dslContext.insertInto(SmsSettings.SMS_SETTINGS)
                .set(SmsSettings.SMS_SETTINGS.SENDER_NAME, "")
                .set(SmsSettings.SMS_SETTINGS.GATEWAY_LOGIN, "")
                .set(SmsSettings.SMS_SETTINGS.GATEWAY_PASSWORD, "")
                .set(SmsSettings.SMS_SETTINGS.CREATED_AT, new Date().getTime())
