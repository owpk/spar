...
public class MerchantCommentsRepositoryImpl implements MerchantCommentsRepository {
    ...
    public List<MerchantComment> list(Integer offset,
        ...
        var res = dslContext
                ...
                .on(Users.USERS.ID.eq(table.USER_ID))
                .leftJoin(Merchants.MERCHANTS)
                .on(Merchants.MERCHANTS.ID.eq(table.MERCHANT_ID))
                .leftJoin(MerchantCommentAnswers.MERCHANT_COMMENT_ANSWERS
                        ...
                        .on(MerchantCommentAnswers.MERCHANT_COMMENT_ANSWERS.OPTION_ID
                                .eq(MerchantCommentsQuestionOptions.MERCHANT_COMMENTS_QUESTION_OPTIONS.ID)))
                .on(table.ID.eq(MerchantCommentAnswers.MERCHANT_COMMENT_ANSWERS.MERCHANT_COMMENT_ID))
                .where();
...
//                .fetchInto(Answer.class);
        if (search != null) {
            res.and(Users.USERS.FIRST_NAME.contains(search)
                    .or(Users.USERS.LAST_NAME.contains(search))
                    .or(Users.USERS.PATRONYMIC_NAME.contains(search))
        ...
        if (dateTimeStart != null) {
            res.and(table.CREATED_AT.greaterOrEqual(dateTimeStart));
        }

        if (dateTimeEnd != null) {
            res.and(table.CREATED_AT.lessOrEqual(dateTimeEnd));
        }

        if (merchantId != null) {
            res.and(table.MERCHANT_ID.in(merchantId));
        }
